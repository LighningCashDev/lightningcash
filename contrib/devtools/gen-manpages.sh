...
LTNCVER=($($LIGHTNINGCASHCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
...
for cmd in $LIGHTNINGCASHD $LIGHTNINGCASHCLI $LIGHTNINGCASHTX $LIGHTNINGCASHQT; do
  cmdname="${cmd##*/}"
  help2man -N --version-string=${LTNCVER[0]} --include=footer.h2m -o ${MANDIR}/${cmdname}.1 ${cmd}
  sed -i "s/\\\-${LTNCVER[1]}//g" ${MANDIR}/${cmdname}.1
done
