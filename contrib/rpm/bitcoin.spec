...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 9123
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 9191
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 19123
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 9123
	%{_sbindir}/semanage port -d -p tcp 9191
	%{_sbindir}/semanage port -d -p tcp 19123
