Lightningcash Core version *0.15.1* is now available from:

  <https://download.lightningcash.org/lightningcash-0.15.1/>
...
A number of changes to the way Lightningcash Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 watts per kB (0.00001 ELTC). This is to prepare for dropping the minimum transaction fee to 0.1 watts per kB (0.0001 ELTC) in the next release.
